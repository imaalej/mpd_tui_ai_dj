...
resolving real track paths against it before anything expensive happens.
...
# `music_directory "/path"` — quotes optional, leading whitespace allowed,
# commented-out lines ignored.
_DIRECTIVE = re.compile(r'^\s*music_directory\s+"?([^"#]+?)"?\s*(?:#.*)?$')
...
def detect_music_directory() -> Tuple[Optional[Path], str]:
    ...
    for candidate in config_file_candidates():
        try:
            ...
            continue
        if value:
            return _expand(value), str(candidate)
...
def validate_music_directory(
    ...
    """
    Prove the directory by resolving real MPD track paths against it.

    `tracks` are paths as MPD reports them (`mpc listall`), i.e. relative to the
    music directory.  Returns (ok, message); the message is written for a user,
    not a developer.
    """
